...
class TestRustE2E(RustE2ETest):
    ...
    def test_search_finds_language_keyword(self, indexed_repo):
        """Test searching for Rust keyword 'fn ' finds results."""
        results = self.search_json("fn ", indexed_repo, regex=True, limit=10)
    ...
    def test_search_json_output_valid(self, indexed_repo):
        ...
        result = self.run_cli(
            ["search", "fn ", "--regex", "--format", "json", "--no-filter"],
            indexed_repo
        )
    ...
    def test_search_csv_output_valid(self, indexed_repo):
        ...
        result = self.run_cli(
            ["search", "fn ", "--regex", "--format", "csv", "-k", "3", "--no-filter"],
            indexed_repo
        )
    ...
    def test_research_finds_related_code(self, indexed_repo):
        ...
        result = self.run_cli(
            ["research", "How does async runtime work?", "--hops", "2"],
            indexed_repo,
            timeout=600
        )
        assert result.returncode == 0
