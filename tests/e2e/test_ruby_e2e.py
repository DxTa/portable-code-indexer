...
class TestRubyE2E(RubyE2ETest):
    ...
    def test_search_finds_language_keyword(self, indexed_repo):
        """Test searching for Ruby keyword 'def' finds results."""
    ...
    def test_search_json_output_valid(self, indexed_repo):
        ...
        result = self.run_cli(
            ["search", "def", "--regex", "--format", "json", "--no-filter"],
            indexed_repo
        )
    ...
    def test_search_csv_output_valid(self, indexed_repo):
        ...
        result = self.run_cli(
            ["search", "def", "--regex", "--format", "csv", "-k", "3", "--no-filter"],
            indexed_repo
        )
    ...
    def test_research_finds_related_code(self, indexed_repo):
        ...
        result = self.run_cli(
            ["research", "How does routing work?", "--hops", "2"],
            indexed_repo,
            timeout=600
        )
        assert result.returncode == 0
